...
public class RotationOverlayPositionScreen extends BaseScreen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        g.centeredText(font, "Drag the overlay box to reposition it. Press Esc to save.", this.width / 2, this.height / 2, HINT_COLOR);
        g.centeredText(font, "Right-click to reset to default (top-right).", this.width / 2, this.height / 2 + 12, HINT_COLOR);
        ...
        int ty = overlayY + 5;
        g.text(font, "§6[Rotation] §aACTIVE", overlayX + 5, ty, 0xFFFFFFFF);
        ty += 10;
        g.text(font, "Yaw:   90.0 -> 45.0", overlayX + 5, ty, 0xFFFFFFFF);
        ty += 10;
        g.text(font, "Pitch: 0.0 -> -30.0", overlayX + 5, ty, 0xFFFFFFFF);
        ty += 10;
        g.fill(overlayX + 5, ty, overlayX + 5 + 100, ty + 4, 0xFF333333);
        g.fill(overlayX + 5, ty, overlayX + 5 + 60, ty + 4, 0xFF00AAFF);
        g.text(font, " 60%", overlayX + 110, ty - 2, 0xFF999999);
    }
